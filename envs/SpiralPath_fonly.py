...
class SpiralPath:
	def __init__(self):
		...
		self.action_dim = 3
		self.observation_dim = 3	#Fx, Fy, Fz
		...
		self.observation_mu = [0, 0, 0]
		self.observation_sigma = [1, 1, 1]
	...
	def reward_func(self, state, action, new_state, done):
		...
		reward = -(new_ob_state[0]**2 + new_ob_state[1]**2) - (1e-9)*np.linalg.norm(action)**2
		return reward
